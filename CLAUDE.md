...
- **分批分类（待实现）**：songs 超阈值时按顺序切批，各批独立分类 → `merge` 节点归一化同义类名 → 统一一轮 HITL；`/classify/start` 异步返回 thread_id，SSE 推批次进度；参数与机制详见 `docs/superpowers/plans/batch-classify-design.md`
